...
const RESULT_VALUE_COUNT: usize = 2;
const RESULT_INPUT_BYTES_INDEX: usize = 0;
const RESULT_OUTPUT_BYTES_INDEX: usize = 1;
const SOURCE_RESULT_VALUE_COUNT: usize = 3;
const SOURCE_RESULT_OUTPUT_STARTED_INDEX: usize = 2;
const SHA_256_BYTE_COUNT: usize = 32;
const NO_EXPECTED_SOURCE_BYTE_LENGTH: jlong = -1;
...
const RESULT_INPUT_LIMIT: jint = 2;
const RESULT_INPUT_LENGTH_MISMATCH: jint = 3;
const RESULT_TIMEOUT: jint = 4;
const RESULT_INVALID_DESCRIPTOR: jint = 5;
const RESULT_INPUT_IO: jint = 6;
const RESULT_OUTPUT_IO: jint = 7;
const RESULT_INTERNAL: jint = 8;
const RESULT_SOURCE_CONFLICT: jint = 9;
const RESULT_SOURCE_UNCERTAIN: jint = 10;
...
/// Owns live export controls behind opaque positive handles.
struct JobRegistry {
    next_handle: i64,
...
impl JobRegistry {
    /// Creates an empty export job registry.
    const fn new() -> Self {
        Self {
...
/// Coordinates cancellation and single execution for one export.
struct JobControl {
    started: AtomicBool,
...
impl Display for BridgeError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UnknownHandle(handle) => write!(formatter, "unknown export handle {handle}"),
            Self::HandleSpaceExhausted => formatter.write_str("export handle space exhausted"),
            Self::JobAlreadyStarted => formatter.write_str("export job already started"),
            Self::RegistryPoisoned => formatter.write_str("export registry poisoned"),
        }
    }
}
...
struct ExportOutcome {
    ...
    output_bytes: u64,
    output_started: bool,
    source_sha256: Option<[u8; SHA_256_BYTE_COUNT]>,
}
...
impl ExportOutcome {
    ...
    const fn from_summary(result_code: jint, summary: ExportSummary) -> Self {
        Self {
            result_code,
    ...
    const fn source_uncertain() -> Self {
        Self {
...
/// Creates an export job control and returns its native handle.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_soupslurpr_beautyxt_exporting_NativeExportWorker_createJob<
    'caller,
>(
...
/// Runs one descriptor-based export and returns a fixed result code.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_soupslurpr_beautyxt_exporting_NativeExportWorker_runJob<'caller>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    handle: jlong,
    input_raw_fd: jint,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_soupslurpr_beautyxt_exporting_NativeExportWorker_runConditionalSourceSaveJob<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    handle: jlong,
    package_raw_fd: jint,
    source_backing_raw_fd: jint,
    source_raw_fd: jint,
    expected_bytes: jlong,
    expected_source_bytes: jlong,
    expected_source_sha256: JByteArray<'caller>,
    timeout_millis: jlong,
    result_values: JLongArray<'caller>,
    ...
    unowned_env
        .with_env(|env| -> Result<jint, BridgeError> {
            if result_values.len(env)? != SOURCE_RESULT_VALUE_COUNT {
                return Err(BridgeError::InvalidArgument("source result array length"));
            }
            if result_sha256.len(env)? != SHA_256_BYTE_COUNT {
                return Err(BridgeError::InvalidArgument("source result digest length"));
            }
            let expected_sha256 = env.convert_byte_array(&expected_source_sha256)?;
            let expected_source =
                decode_source_expectation(expected_source_bytes, &expected_sha256)?;
            let job = lock_registry()?.get(handle)?;
            job.start()?;
            let outcome = run_conditional_source_save(
                &job,
                package_raw_fd,
                source_backing_raw_fd,
                source_raw_fd,
                expected_bytes,
                expected_source.as_ref(),
                timeout_millis,
            );
            write_source_outcome(env, &result_values, &result_sha256, &outcome)?;
            Ok(outcome.result_code)
        })
        .resolve::<ThrowRuntimeExAndDefault>()
}

/// Runs one bounded read-only source verification.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_soupslurpr_beautyxt_exporting_NativeExportWorker_runSourceVerificationJob<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|env| -> Result<jint, BridgeError> {
            ...
            let expected_sha256 = env.convert_byte_array(&expected_source_sha256)?;
            let expected_source =
                decode_exact_source_version(expected_source_bytes, &expected_sha256)?;
            let job = lock_registry()?.get(handle)?;
            job.start()?;
            let outcome =
                run_source_verification(&job, source_raw_fd, &expected_source, timeout_millis);
            write_source_statistics(env, &result_values, &outcome)?;
            Ok(outcome.result_code)
        })
        .resolve::<ThrowRuntimeExAndDefault>()
}

/// Runs one bounded read-only source inspection.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_soupslurpr_beautyxt_exporting_NativeExportWorker_runSourceInspectionJob<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|env| -> Result<jint, BridgeError> {
            ...
            job.start()?;
            let outcome =
                run_source_inspection(&job, source_raw_fd, maximum_source_bytes, timeout_millis);
            write_source_outcome(env, &result_values, &result_sha256, &outcome)?;
            Ok(outcome.result_code)
        })
        .resolve::<ThrowRuntimeExAndDefault>()
...
/// Writes one conditional source outcome into fixed JNI arrays.
fn write_source_outcome(
    env: &jni::Env<'_>,
    result_values: &JLongArray<'_>,
    result_sha256: &JByteArray<'_>,
    outcome: &ExportOutcome,
) -> Result<(), BridgeError> {
    write_source_statistics(env, result_values, outcome)?;
    let digest = outcome.source_sha256.unwrap_or([0; SHA_256_BYTE_COUNT]);
    let signed_digest = digest.map(|byte| i8::from_ne_bytes([byte]));
    result_sha256.set_region(env, 0, &signed_digest)?;
    Ok(())
}

/// Writes fixed source statistics and destructive-boundary evidence through JNI.
fn write_source_statistics(
    env: &jni::Env<'_>,
    result_values: &JLongArray<'_>,
    outcome: &ExportOutcome,
) -> Result<(), BridgeError> {
    let mut values = [0; SOURCE_RESULT_VALUE_COUNT];
    values[RESULT_INPUT_BYTES_INDEX] = jlong::try_from(outcome.input_bytes)
        .map_err(|_| BridgeError::InvalidArgument("input byte count"))?;
    values[RESULT_OUTPUT_BYTES_INDEX] = jlong::try_from(outcome.output_bytes)
        .map_err(|_| BridgeError::InvalidArgument("output byte count"))?;
    values[SOURCE_RESULT_OUTPUT_STARTED_INDEX] = jlong::from(outcome.output_started);
    result_values.set_region(env, 0, &values)?;
    Ok(())
}

/// Requests idempotent cancellation of an export job.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_soupslurpr_beautyxt_exporting_NativeExportWorker_cancelJob<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
/// Destroys an export job handle.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_soupslurpr_beautyxt_exporting_NativeExportWorker_destroyJob<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
/// Identifies one exact source byte sequence.
#[derive(Clone, Copy, Debug)]
struct ExactSourceVersion {
    byte_length: u64,
...
fn decode_exact_source_version(
    expected_source_bytes: jlong,
    expected_source_sha256: &[u8],
...
fn run_conditional_source_save(
    job: &Arc<JobControl>,
...
fn bounded_snapshot_bytes(expected_bytes: jlong) -> Result<u64, ()> {
...
fn validated_deadline(timeout_millis: jlong) -> Result<Instant, BridgeError> {
    let timeout_millis =
...
/// Runs one validated export without propagating document failures through JNI.
fn run_export(
    job: &Arc<JobControl>,
    input_raw_fd: RawFd,
    output_raw_fd: RawFd,
...
fn descriptor_seals(file: &File) -> Result<jint, BridgeError> {
...
fn set_nonblocking(file: &File, flags: jint) -> Result<(), BridgeError> {
...
const fn result_code_for_error(error: &ExportError) -> jint {
    match error {
...
/// Converts a nonnegative Java long into an unsigned value.
fn nonnegative_u64(value: jlong) -> Result<u64, ()> {
    u64::try_from(value).map_err(|_| ())
}

/// Converts a positive Java long into an unsigned value.
...
mod tests {
    ...
    fn create_sealed_input(bytes: &[u8]) -> File {
        ...
        create_sealed_memfd("beautyxt-export-jni-test", &package_bytes)
    }
    ...
    fn create_large_sparse_package(
        ...
        let package = create_sealed_memfd("beautyxt-export-jni-large-package", &package_bytes);
        let mut source_backing = create_unsealed_memfd("beautyxt-export-jni-large-backing", b"");
    ...
    fn test_output_path(case_name: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "beautyxt-export-jni-{case_name}-{}.tmp",
            std::process::id()
        ))
    }
