...
const RESULT_VALUE_COUNT: usize = 5;
const RESULT_INPUT_BYTES_INDEX: usize = 0;
const RESULT_PACKET_BYTES_INDEX: usize = 1;
const RESULT_BLOCK_COUNT_INDEX: usize = 2;
const RESULT_SPAN_COUNT_INDEX: usize = 3;
const RESULT_FLAGS_INDEX: usize = 4;
...
const RESULT_RENDER_LIMIT: jint = 6;
...
impl Display for BridgeError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UnknownHandle(handle) => write!(formatter, "unknown markdown handle {handle}"),
            Self::HandleSpaceExhausted => formatter.write_str("markdown handle space exhausted"),
            Self::JobAlreadyStarted => formatter.write_str("markdown job already started"),
            Self::RegistryPoisoned => formatter.write_str("markdown registry poisoned"),
        }
    }
}
...
/// Creates a Markdown job control and returns its native handle.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_soupslurpr_beautyxt_markdown_NativeMarkdownWorker_createJob<
    'caller,
>(
...
/// Runs one descriptor-based Markdown render and returns a fixed result code.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_soupslurpr_beautyxt_markdown_NativeMarkdownWorker_runJob<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|env| -> Result<jint, BridgeError> {
            ...
            values[RESULT_PACKET_BYTES_INDEX] = jlong::try_from(outcome.packet_bytes)
                .map_err(|_| BridgeError::InvalidArgument("packet byte count"))?;
            values[RESULT_BLOCK_COUNT_INDEX] = jlong::from(outcome.block_count);
            values[RESULT_SPAN_COUNT_INDEX] = jlong::from(outcome.span_count);
            values[RESULT_FLAGS_INDEX] = jlong::from(outcome.flags);
...
/// Requests idempotent cancellation of a Markdown job.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_soupslurpr_beautyxt_markdown_NativeMarkdownWorker_cancelJob<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
/// Destroys a Markdown job handle.
#[unsafe(no_mangle)]
pub extern "system" fn Java_dev_soupslurpr_beautyxt_markdown_NativeMarkdownWorker_destroyJob<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
/// Maps a core render error to the stable JNI result protocol.
const fn result_code_for_render_error(error: RenderError) -> jint {
    match error {
...
/// Maps a cooperative interruption to the stable JNI result protocol.
const fn result_code_for_interruption(interruption: RenderInterruption) -> jint {
    match interruption {
...
mod tests {
    ...
    fn test_path(case_name: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "beautyxt-markdown-jni-{case_name}-{}.tmp",
            std::process::id()
        ))
    }
}
